...
export default function home() {
  ...
  async function initNetworkListener() {
    if (Capacitor.isNativePlatform()) {
      ...
      setStatus(netStatus.connected ? 'Online' : 'Offline');
  ...
  useEffect(() => {
    if (status === 'Offline') setTabIndex(1);
  }, [status]);
  ...
  useEffect(() => {
    const newUpdate = sessionStorage.getItem('isNewUpdate');
    if (newUpdate) setIsNewUpdate(true);
  }, []);
  ...
  const posts_icons = [
    ...
    <div 
      ...
      onClick={() => user ? router.push('/account') : setNoUser(true)}
  ...
  return(
    <div>
      <Header 
        ...
        rightIcons={user ? (tabIndex == 1 ? notes_icons : posts_icons) : null}
        isTransparent={true}
        blur={'10px'}
      />
        ...
        <Footer 
          icons={[
            <List 
              onClick={() => switchTab(1)}
              fill
              className={tabIndex == 1 ? styles.tabIconActive : styles.tabIcon} 
              size={20}
            />,
            <Heart 
              onClick={() => switchTab(2)} 
